...
**Date:** 2026-09-02  
**Canonical continuation branch:** `main` after PR #21 merge  
**Status:** `ARCHITECTURE_V4_FROZEN__3D_EQUIVALENT_MECHANICS__DIRECTIONAL_2D_2P5D_PRODUCT__PRODUCT_V3_TYPED__SUPPORT_ADMISSION_FIXED__SINGLE_FAMILY_E2E_IS_NEXT__GENERALIZATION_BLOCKED__NO_LEARNED_OPTIMIZER_AUTHORIZED`

This file is continuation authority only on `main`.

## One-line state

`Historical compiler/runtime maximum audited -> current migration reconciled -> Geppetto/Arachne R6 boundaries frozen -> MWB0/MWB1 PASS -> final Drive/Library/GitHub adversarial audit closed -> Architecture V4 freezes 3D-equivalent mechanical evidence versus directional 2D/2.5D product ontology -> CanonicalPuppetGraph.v3 typed with exact-eight directional renderables, appearance/completion firewall, capability contract, puppet-local motion and sibling proof bundle -> next objective is one real-family end-to-end animated puppet PASS -> generalization remains blocked.`
...
```text
8 neutral 1024 views + exact cameras
...
Models produce evidence/proposals. Compiler owns canonical IDs, admissibility, product state, proof/export boundaries. Runtime is a projection only.
...
## 2. V4 source contract — TYPED

Current V4 source adds:
...
Legacy V1/V2 product/proof paths remain for compatibility/regression evidence but are not the final V4 composition target.
...
ONE CLEAN REAL FAMILY
 ...
 -> Geppetto fit -> coherent QualifiedSkeletonIR.v2
 -> SkinFieldCodec + Arachne fit -> QualifiedSkinIR
...
1. Freeze clean 1024 FIT8 candidate manifest and choose the initial family without family-specific architecture changes.
...
11. Only after E2E PASS freeze the heterogeneous FIT8 set and begin generalization work.

## 5. Existing closed scientific / migration facts retained

- DTB-ND1 robust local-plane clean non-inferiority closed with `0.00250 <= epsilon_critical < 0.00275 RMS` for the audited consumer profile.
...
- Geppetto/Arachne R0-R5 external-reference audit closed; R6 consumer boundaries remain useful.
...
**IRIS/Geppetto/Arachne learned optimizer steps: not globally authorized by architecture alone; open only under the single-family FIT contract and prospective frozen candidate/prereg for the exact rung being executed.**  
...
> one clean real family enters as eight 1024 observations and exits as a convincingly jointed, properly skinned, deformable, source-looking eight-direction puppet that visibly performs a preset animation through the actual canonical compiler/proof/runtime route.
