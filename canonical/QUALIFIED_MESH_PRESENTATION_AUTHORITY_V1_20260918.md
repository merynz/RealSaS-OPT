...
Stages 24–29 are now executable typed adapters: `MECHANICAL_PARTITION_QUALIFIED -> DEFORMATION_CAPABILITY_ENVELOPE -> MESH_CANDIDATE_BUILD -> QUALIFIED_MESH_GATE -> QUALIFIED_MESH_SKIN_TRANSFER -> CANONICAL_PUPPET_STATE_SEALED`. Their dependency edges are exact consumed-authority edges rather than ordinal convenience edges, and the orchestrator invalidates only the changed stage plus its transitive dependency subgraph. Stage 30 `QUALIFIED_PRESENTATION_GRAPH` remains intentionally `UNBOUND` until automatic slot/component ownership production is separately closed. Executable adapter status is not a Knight PASS; the active run remains 0/40 until exact run artifacts pass these stages.
