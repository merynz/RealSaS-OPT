...
"""Mainline stages 03-08: source mechanical audit, full-subject admission and observation authority."""
...
from compiler.realsas_compiler_services.orchestrator.adapters.product_mesh_v1 import (
    _load_file_ref, _stage_output_payload, _write_ir,
)
...
def _observation_rows(ctx:dict,key:str,ref_key:str):
    ...
    for row in rows:
        ...
        path=_load_file_ref(dict(row.get(ref_key) or {}),json_required=False)
...
def audit_source_mechanics_stage(ctx:dict)->dict:
    source=_stage_output_payload(ctx,"01_SOURCE_BYTES_SEALED","RealSaS.SourceByteSeal.v1")
    ...
    root=ctx["run_root"]/"artifacts"/"03_SOURCE_MECHANICAL_AUDIT"
    return {"status":"PASS","outputs":[_write_ir(root/"source_mechanical_audit.json",audit,authority_class="SOURCE_MECHANICAL_AUDIT")],
...
def admit_full_subject_stage(ctx:dict)->dict:
    audit=source_mechanical_audit_from_dict(
        _stage_output_payload(ctx,"03_SOURCE_MECHANICAL_AUDIT","RealSaS.SourceMechanicalAuditIR.v1")
    )
    ...
    root=ctx["run_root"]/"artifacts"/"04_FULL_SUBJECT_ADMISSION"
    return {"status":"PASS","outputs":[_write_ir(root/"full_subject_admission.json",value,authority_class="FULL_SUBJECT_ADMISSION")],
...
def materialize_observation_render_stage(ctx:dict)->dict:
    admission=full_subject_admission_from_dict(
        _stage_output_payload(ctx,"04_FULL_SUBJECT_ADMISSION","RealSaS.FullSubjectAdmissionIR.v1")
    )
    ...
    root=ctx["run_root"]/"artifacts"/"06_OBSERVATION_RENDER_8VIEW"
    return {"status":"PASS","outputs":[_write_ir(root/"observation_render_set.json",value,authority_class="EXACT_OBSERVATION_RENDER_SET")],
...
def qualify_observation_contract_stage(ctx:dict)->dict:
    ...
    renders=observation_render_set_from_dict(
        _stage_output_payload(ctx,"06_OBSERVATION_RENDER_8VIEW","RealSaS.ObservationRenderSetIR.v1")
    )
    masks=_observation_rows(ctx,"source_foreground_masks","mask")
    ...
    root=ctx["run_root"]/"artifacts"/"07_OBSERVATION_CONTRACT_QUALIFIED"
    return {"status":"PASS","outputs":[_write_ir(root/"qualified_observation_set.json",value,authority_class="QUALIFIED_OBSERVATION_SET")],
...
def qualify_normalization_domain_stage(ctx:dict)->dict:
    observation_payload=_stage_output_payload(ctx,"07_OBSERVATION_CONTRACT_QUALIFIED","RealSaS.QualifiedObservationSetIR.v1")
    cameras=qualified_camera_set_from_dict(
        _stage_output_payload(ctx,"05_CAMERA_CONTRACT_SOLVED","RealSaS.QualifiedCameraSetIR.v1")
    )
    ...
    root=ctx["run_root"]/"artifacts"/"08_NORMALIZATION_DOMAIN_QUALIFIED"
    return {"status":"PASS","outputs":[_write_ir(root/"normalization_domain.json",value,authority_class="QUALIFIED_NORMALIZATION_DOMAIN")],
