...
from .mesh_demo_v1 import DemoMeshConfig, build_view_local_identity_mesh_candidate
...
def _load_image_only(path: str|Path):
    d=np.load(path,allow_pickle=False);allowed={"rgba","yaw_deg"};extra=set(d.files)-allowed
...
def run_final_image_only_inference(request: ImageOnlyInferenceRequest, *, device: str="auto") -> dict:
    assert_image_only_request(request);out=Path(request.output_dir);out.mkdir(parents=True,exist_ok=True);write_firewall_receipt(request,out/"INFERENCE_FIREWALL_RECEIPT_V1.json")
    ...
    geppetto,_,_=_load_model(request.geppetto_checkpoint,GeppettoDemoV1,GeppettoDemoConfig,"GEPPETTO",dev)
    ...
    evidence,groups=build_observation_evidence_and_persistence(raw,images,cameras,cfg=icfg);surface=build_surface_from_persistence(evidence,groups)
    if not surface.surface_nodes:raise RuntimeError("FINAL_INFERENCE_EMPTY_SURFACE")
    skeleton=qualify_skeleton(surface,geppetto.propose(surface,device=dev));skin=qualify_skin(surface,skeleton,arachne.propose(surface,skeleton,device=dev),max_influences=acfg.top_k)

    view_index=0;camera_contract=cameras[view_index]
    candidate=build_view_local_identity_mesh_candidate(surface,view_index=view_index,camera_contract=camera_contract,cfg=DemoMeshConfig(max_vertices=768))
    mesh=qualify_identity_subset_mesh(surface,candidate);mesh_skin=bind_identity_mesh_skin(surface,skeleton,skin,mesh)
    ...
    runtime_req=build_historical_runtime_request(skeleton=skeleton,mesh=mesh,mesh_skin=mesh_skin,camera=camera_contract,frames=motion,raster_size=(icfg.model_resolution,icfg.model_resolution),arap_enabled=True)
    (out/"RUNTIME_REQUEST_V1.json").write_text(json.dumps(runtime_req,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    ...
    worker=Path(__file__).with_name("historical_runtime_worker_v1.py");runtime=run_historical_reference_runtime(runtime_req,source_zip=source_zip,worker_script=worker)

    frames=runtime.output["frames"];finite=all(np.isfinite(np.asarray(f["vertices_xy"],dtype=float)).all() for f in frames);no_mutation=runtime.output.get("compiler_artifacts_mutated") is False;proof_pass=bool(finite and no_mutation and len(frames)==48)
    ...
    (out/"HISTORICAL_RUNTIME_FRAMES_V1.json").write_text(json.dumps(runtime.output,indent=2,sort_keys=True)+"\n",encoding="utf-8");(out/"CANONICAL_PRODUCT_V2.json").write_text(json.dumps(product.to_dict(),indent=2,sort_keys=True)+"\n",encoding="utf-8");(out/"PROOF_FRAME_V1.json").write_text(json.dumps(proof.to_dict(),indent=2,sort_keys=True)+"\n",encoding="utf-8");(out/"RUNTIME_PACKAGE_IR_V1.json").write_text(json.dumps(runtime_pkg.to_dict(),indent=2,sort_keys=True)+"\n",encoding="utf-8")
    report={"schema":"RealSaS.SingleSpecimenImageOnlyE2EClosure.v1","status":"PASS" if proof_pass else "FAIL","generalization_claim":False,"image_only_final_inference":True,"manual_output_injection":False,"device":str(dev),"surface_node_count":len(surface.surface_nodes),"joint_count":len(skeleton.joints),"skin_row_count":len(skin.rows),"mesh_vertex_count":len(mesh.vertices),"mesh_face_count":len(mesh.faces),"runtime_frame_count":len(frames),"historical_runtime_source_sha256":runtime.source_zip_sha256,"product_state_hash":product.product_state_hash,"proof_passed":proof.passed}
