...
def _mesh_rest_xy(mesh: QualifiedEditableMeshIR, raster_size: tuple[int, int]) -> tuple[list[list[float]], dict[str, int]]:
    width,height=map(int,raster_size)
    if width<=0 or height<=0:raise ValueError("RUNTIME_BRIDGE_BAD_RASTER_SIZE")
    rows=[]; index={}
    for i,vertex in enumerate(mesh.vertices):
        ...
        px,py=map(float,xy)
        gx=((px+0.5)/width)*2.0-1.0
        gy=((py+0.5)/height)*2.0-1.0
...
def build_generic_articulation_sweep(
    ...
    *,
    ...
    """
    if frame_count < 2:raise ValueError("frame_count must be >=2")
    by_id={j.canonical_joint_id:j for j in skeleton.joints}
    if skeleton.root_id not in by_id:raise ValueError("qualified skeleton root missing")
    ...
    for j in skeleton.joints:
        if j.parent_canonical_id is not None:children[j.parent_canonical_id].append(j.canonical_joint_id)
    depth={skeleton.root_id:0};queue=[skeleton.root_id]
    while queue:
        p=queue.pop(0)
        for c in sorted(children[p]):depth[c]=depth[p]+1;queue.append(c)
    max_depth=max(depth.values()) if depth else 0
    frames=[]
    for fi in range(frame_count):
        t=fi/(frame_count-1);phase=2*pi*t;state={}
        for jid in sorted(by_id):
            j=by_id[jid];xy=project_point_to_normalized_raster(j.position,camera)
            ...
            depth_frac=depth.get(jid,1)/max(1,max_depth);amp=max_rotation_deg*(0.35+0.65*depth_frac)
...
def build_historical_runtime_request(
    *,
    ...
    if mesh.mesh_lineage_hash != mesh_skin.mesh_binding_hash:raise ValueError("RUNTIME_BRIDGE_MESH_SKIN_LINEAGE_MISMATCH")
    vertices,index=_mesh_rest_xy(mesh,raster_size)
    triangles=[]
    for face in mesh.faces:
        if len(face)==3:triangles.append([index[x] for x in face])
    if not triangles:raise ValueError("RUNTIME_BRIDGE_REQUIRES_TRIANGLES")
    skin_by_vertex={row.canonical_mesh_vertex_id:dict(row.influences) for row in mesh_skin.rows};weights=[]
    for vertex in mesh.vertices:
        row=skin_by_vertex.get(vertex.canonical_mesh_vertex_id)
        if not row:raise ValueError(f"RUNTIME_BRIDGE_MISSING_WEIGHT_ROW:{vertex.canonical_mesh_vertex_id}")
        weights.append({str(k):float(v) for k,v in row.items()})
    joints=[{"joint_id":joint.canonical_joint_id,"parent_id":joint.parent_canonical_id,"anchor_xy":list(project_point_to_normalized_raster(joint.position,camera))} for joint in skeleton.joints]
    return {
        "schema":"RealSaS.DemoHistoricalRuntimeRequest.v1","view_index":int(mesh.view_index),
        "skeleton":{"root_id":skeleton.root_id,"joints":joints},
        "mesh":{"vertices_xy":vertices,"triangles":triangles,"weights":weights},
        "frames":frames,"clip_intent":"generic_articulation_sweep",
        "deformation":{"arap_enabled":bool(arap_enabled),"arap_anchor_weight":100000.0,"arap_iterations":4},
        "generalization_claim":False,"manual_output_injection":False,
    }
