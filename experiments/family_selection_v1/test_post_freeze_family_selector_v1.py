...
def candidate(i: int, *, eligible: bool = True) -> PrefitFamilyCandidateV1:
    ...
    return PrefitFamilyCandidateV1(
        asset_id=f"asset_blind_{i:04d}", source_family_id=f"family_blind_{i:04d}",
        master_native_1024=True, master_fit_admit=True, iris_truth_capable=True,
        geppetto_truth_capable=True, arachne_truth_capable=True, single_pose_core_eligible=True,
        exact_camera_raster_authority=True, explicit_source_textured_rgba=True,
        prefit_truth_eligibility_pass=True, prefit_truth_eligibility_sha256=h,
        visual_character_only_pass=eligible, visual_no_render_artifact_pass=True,
        visual_no_dominant_geometric_block_pass=True, visual_clear_character_silhouette_pass=True,
        visual_audit_id="visual-audit-prefit-v1", visual_audit_sha256=h, candidate_authority_sha256=h,
    )
...
def truth_report(row: PrefitFamilyCandidateV1, *, passed: bool = True) -> PrefitFamilyTruthEligibilityV1:
    return PrefitFamilyTruthEligibilityV1(
        schema="RealSaS.PrefitFamilyTruthEligibility.v1", policy_id=TRUTH_POLICY_ID,
        asset_id=row.asset_id, master_manifest_sha256="b" * 64, observation_authority_sha256="c" * 64,
        image_filename="source_textured_rgba.png", pass_prefit_truth_eligibility=passed,
        observed_triangle_count=12, observed_degenerate_triangle_count=0,
        stable_triangle_view_pair_count=80, raster_unstable_excluded_triangle_view_pair_count=16,
        hull_miss_stable_triangle_view_pair_count=0, fully_raster_unstable_observed_triangle_count=0,
        minimum_triangle_weighted_normal_polarity_purity=1.0,
        minimum_pixel_weighted_normal_polarity_purity=1.0, views=(), policy={"frozen": True},
        scientific_fit_steps=0, postfit_information_consumed=False,
        source_mesh_used_for_model_input=False, teacher_truth_used_for_model_input=False,
...
def frozen_authority(monkeypatch):
    monkeypatch.setattr(selector, "require_family_selection_authority", lambda root: {
        "living_source_fingerprint_sha256": FREEZE_SHA,
        "selection_apparatus_fingerprint_sha256": SELECTION_SHA,
    })
...
def test_selection_is_deterministic_prefit_only_with_typed_truth_authority():
    ...
    assert a.selection_apparatus_fingerprint_sha256 == SELECTION_SHA
...
def test_truth_pass_boolean_cannot_bypass_missing_typed_report():
    rows = tuple(candidate(i) for i in range(8)); report_map = reports(rows); del report_map[rows[3].asset_id]
...
def test_truth_report_sha_mismatch_fails_closed():
    rows = tuple(candidate(i) for i in range(8)); report_map = reports(rows)
...
def test_failed_or_postfit_contaminated_truth_report_fails_closed():
    rows = tuple(candidate(i) for i in range(8)); failed = reports(rows)
...
def test_postfit_fields_are_rejected_not_ignored():
    raw = candidate(1).__dict__.copy(); raw["fit_loss"] = 0.00001
...
def test_unknown_fields_are_rejected_to_keep_selection_schema_closed():
    raw = candidate(1).__dict__.copy(); raw["pretty_score"] = 0.99
