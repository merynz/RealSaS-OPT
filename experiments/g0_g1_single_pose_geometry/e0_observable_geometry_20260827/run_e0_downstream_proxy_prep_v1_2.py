#!/usr/bin/env python3
"""Preparation of compact D0/D1/D2 packs after camera-recovery estimator V1.2.

D0/D1/D2 semantics, split, admission, models and optimizer schedules are unchanged:
- same frozen split;
- same source-hash checks;
- same exact build_compact_pack(..., anchor_count=512);
- same D0/D1/D2 semantics and MUTUAL_P003 admission.

V1.2 retains the V1.1 hardened Drive publication path and additionally binds every
resume record to the exact e0_geometry.py SHA, so packs made under an older camera
recovery estimator can never be silently resumed.
"""
...
CODE_VERSION = "E0_DOWNSTREAM_PROXY_PREP_V1_2_CAMERA_RECOVERY_HASH_BOUND"
...
def try_resume(pp: Path, rp: Path, builder_sha: str, geometry_sha: str):
    try:
        ...
        if old.get("builder_source_sha256") != builder_sha:
            return None
        if old.get("geometry_source_sha256") != geometry_sha:
            return None
...
def main():
    ...
    ap.add_argument("--geometry-source-sha256", required=True)
    a = ap.parse_args()
    ...
    print("PREP V1.2 POPULATION", len(split["train_ids"]), len(split["selection_ids"]), len(split["calibration_ids"]), flush=True)
    ...
    for i, aid in enumerate(ids, 1):
        ...
        old = try_resume(pp, rp, a.builder_source_sha256, a.geometry_source_sha256)
        if old is not None:
        ...
        report = {
            ...
            "geometry_source_sha256": a.geometry_source_sha256,
