...
def eval_arachne_r6_a1_v1(
    ...
    *,
    ...
    model.eval(); out=model(sf,jf,sm,jm,pi,pg,pm); teacher_latents=model.codec.encode_teacher_weights(sf,jf,teacher_weights,sm,jm); valid=jm[...,None].expand_as(out.joint_latent_mean); residual=out.joint_latent_mean-teacher_latents; ls=out.joint_latent_log_sigma; latent_nll=(0.5*torch.exp(-2*ls)*residual.square()+ls)[valid].mean(); reconstruction=skin_field_codec_loss_v1(out.decoded_weights,teacher_weights,sm,jm); deformation=codec_deformation_loss_v1(out.decoded_weights,teacher_weights,rest_points,probe_transforms,sm,jm); simplex=out.decoded_weights.sum(-1); metrics={"latent_nll":float(latent_nll.cpu()),"reconstruction":float(reconstruction["total"].cpu()),"deformation_rms":float(deformation["deformation_rms"].cpu()),"uncertainty_mean":float(torch.exp(ls[valid]).mean().cpu()),"max_simplex_residual":float((simplex[sm]-1.0).abs().max().cpu()),"negative_weight_count":int((out.decoded_weights[(sm[:,:,None]&jm[:,None,:])]<0).sum().cpu())}
    if any(not math.isfinite(float(v)) for v in metrics.values()): raise ValueError("non-finite Arachne A1 metrics")
    return metrics
