...
For each source observation and target view, target candidates are **alpha-supported dense 128×128 image-grid pixels**. Ground-truth track identity is never a candidate input; track truth is used only to choose evaluation queries and score the returned coordinates.

1. exact-camera row corridor defines the legal dense search domain (`|dy| <= 0.030` normalized);
...
- no GT track-node candidate list; candidate search is dense over alpha-supported image pixels;
