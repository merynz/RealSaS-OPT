...
def move(batch,device,input_resolution):
    ...
    return out
def eval_split(model,loader,device,input_resolution):
    """Selection objective is invariant across epochs: always full post-warmup loss."""
    model.eval(); sums={}; n=0
    with torch.no_grad():
        for batch in loader:
            batch=move(batch,device,input_resolution); out=model(batch['images'],batch['yaw_deg']); parts=total_loss(out,batch,epoch=WARMUP_EPOCHS,warmup_epochs=WARMUP_EPOCHS)
            for k,v in parts.items(): sums[k]=sums.get(k,0.0)+float(v.detach())
            n+=1
    return {k:v/max(n,1) for k,v in sums.items()}
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--cache-manifest',required=True); ap.add_argument('--out',required=True)
    ap.add_argument('--epochs',type=int,default=24); ap.add_argument('--batch-size',type=int,default=1); ap.add_argument('--lr',type=float,default=5e-5); ap.add_argument('--weight-decay',type=float,default=1e-4)
    ap.add_argument('--grad-clip',type=float,default=2.0); ap.add_argument('--input-resolution',type=int,default=256); ap.add_argument('--corr-samples',type=int,default=256); ap.add_argument('--workers',type=int,default=2); ap.add_argument('--seed',type=int,default=20260823); ap.add_argument('--device',default='cuda'); ap.add_argument('--resume',action='store_true')
    ...
    model=IRISControlledV1().to(device); opt=torch.optim.AdamW(model.parameters(),lr=a.lr,weight_decay=a.weight_decay); scaler=torch.amp.GradScaler('cuda',enabled=device.type=='cuda')
    ...
    meta={'schema':'RealSaS.IRISControlledV1.TrainingRun.v1_1','cache_manifest':str(Path(a.cache_manifest).resolve()),'cache_manifest_sha256':sha256(a.cache_manifest),'parameters':count_parameters(model),'args':vars(a),'warmup_epochs':WARMUP_EPOCHS,'selection_objective':'FULL_POST_WARMUP_OBJECTIVE_AT_EVERY_EPOCH','sealed_splits_opened':False}
    ...
    for epoch in range(start_epoch,a.epochs):
        ...
        for batch in fit_loader:
            ...
            with torch.amp.autocast(device_type=device.type,enabled=device.type=='cuda'):
                pred=model(batch['images'],batch['yaw_deg']); parts=total_loss(pred,batch,epoch,warmup_epochs=WARMUP_EPOCHS); loss=parts['total']
            ...
            for k,v in parts.items(): sums[k]=sums.get(k,0.0)+float(v.detach())
            n+=1
        train={k:v/max(n,1) for k,v in sums.items()}; val=eval_split(model,tune_loader,device,a.input_resolution)
        row={'epoch':epoch,'train_objective':train,'tune_selection_full_objective':val,'elapsed_sec':time.time()-t}; hist.append(row); score=val['total']
        ...
        atomic_json(outdir/'history.json',{'schema':'RealSaS.IRISControlledV1.History.v1_1','best_tune_total':best,'epochs':hist})
