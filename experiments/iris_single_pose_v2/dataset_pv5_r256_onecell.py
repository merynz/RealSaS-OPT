...
class PV5R256OneCellDataset(Dataset):
    def __init__(self,cache_manifest:str):
        ...
        if m.get('record_count')!=1 or m.get('asset_style_cells')!=1 or m.get('camera_json_consumed') is not False:
            raise RuntimeError('invalid R256 one-cell cache')
        r=m['records'][0]
        if r.get('split')!='FIT' or r.get('style')!='cel_clean':
            raise RuntimeError('one-cell membership drift')
        self.r=r
