...
def _masked_multi_positive_ce(logits, pos, valid):
    ...
    if not has_pos.any(): return logits.sum() * 0.0
...
def pairwise_coarse_objective(zs, zt, p, temperature=0.08, same_radius=0.003, far_radius=0.025, margin=0.10):
    zs = F.normalize(zs, dim=-1, eps=1e-8); zt = F.normalize(zt, dim=-1, eps=1e-8)
    logits = (zs @ zt.T) / temperature
    dist = torch.cdist(p.float(), p.float()); pos = dist <= same_radius; valid = (dist >= far_radius) | pos
    dual = 0.5 * (_masked_multi_positive_ce(logits, pos, valid) + _masked_multi_positive_ce(logits.T, pos.T, valid.T))
    sim = zs @ zt.T; neg_mask = dist >= far_radius
    ...
    pab = F.softmax(logits.masked_fill(~valid, -1e4), dim=1); pba = F.softmax(logits.T.masked_fill(~valid.T, -1e4), dim=1)
    reciprocal = -torch.log(torch.diagonal(pab @ pba).clamp_min(1e-12)).mean()
...
def observation_supcon(z, p, vis, temperature=0.08, same_radius=0.003, far_radius=0.025, max_obs=1024):
    ...
    if idx.shape[0] < 3: return z.sum() * 0.0
    if idx.shape[0] > max_obs: idx = idx[:max_obs]
    f = F.normalize(z[idx[:, 0], idx[:, 1]], dim=-1, eps=1e-8); pp = p[idx[:, 1]].float()
    sim = (f @ f.T) / temperature; dist = torch.cdist(pp, pp)
    eye = torch.eye(len(f), device=f.device, dtype=torch.bool)
    pos = (dist <= same_radius) & ~eye; valid = ((dist >= far_radius) | pos) & ~eye
    return _masked_multi_positive_ce(sim, pos, valid)


def coarse_correspondence_loss(outputs, batch):
    xy = batch["track_xy"].permute(0, 2, 1, 3).contiguous(); vis = batch["track_visible"].bool(); p = batch["track_p"]
    ...
    for b in range(z.shape[0]):
        ...
        for s in range(z.shape[1]):
            for t in range(s + 1, z.shape[1]):
                ...
                d, h, r = pairwise_coarse_objective(z[b, s, m], z[b, t, m], p[b, m]); duals.append(d); hards.append(h); recips.append(r)
    zero = z.sum() * 0.0; mean = lambda xs: torch.stack(xs).mean() if xs else zero
    return {"Zc_multi_positive": mean(globals_), "Zc_dual": mean(duals), "Zc_hard_margin": mean(hards), "Zc_reciprocal_soft": mean(recips)}
...
def fine_local_loss(outputs, batch, radius_cells=4, temperature=0.05, max_tracks_per_pair=64):
    zf = outputs["Z_fine"]; bsz, views, dim, hf, wf = zf.shape
    xy = batch["track_xy"]; vis = batch["track_visible"].bool(); offsets = _fine_offsets(radius_cells, zf.device, zf.dtype)
    delta = offsets.clone(); delta[:, 0] *= 2.0 / float(wf); delta[:, 1] *= 2.0 / float(hf)
    zero_idx = ((offsets[:, 0] == 0) & (offsets[:, 1] == 0)).nonzero(as_tuple=False).flatten()
    if zero_idx.numel() != 1: raise RuntimeError("fine offset lattice has no unique zero")
    zero_idx = int(zero_idx.item()); losses, top1s, errs = [], [], []
    for b in range(bsz):
        for s in range(views):
            for t in range(s + 1, views):
                ...
                ids = ids[:max_tracks_per_pair]; src = xy[b, ids, s]; truth = xy[b, ids, t]
                qs = sample_field(zf[b:b+1, s:s+1], src[None, None])[0, 0]
                cand = truth[:, None, :] + delta[None]; inb = (cand.abs() <= 1.0).all(-1)
                zcand = sample_field(zf[b:b+1, t:t+1], cand.clamp(-1, 1).reshape(1, 1, -1, 2))[0, 0].reshape(len(ids), len(delta), dim)
                logits = (F.normalize(qs, dim=-1)[:, None, :] * F.normalize(zcand, dim=-1)).sum(-1) / temperature
                logits = logits.masked_fill(~inb, -1e4); valid = inb[:, zero_idx]
                if not valid.any(): continue
                lv = logits[valid]; labels = torch.full((int(valid.sum()),), zero_idx, device=zf.device, dtype=torch.long)
                losses.append(F.cross_entropy(lv, labels)); pred = lv.argmax(-1)
                top1s.append((pred == zero_idx).float().mean()); errs.append(offsets[pred].float().norm(dim=-1).mean())
    zero = zf.sum() * 0.0
    return {"Zf_local": torch.stack(losses).mean() if losses else zero,
...
def track_p_consistency_loss(outputs, batch):
    xy = batch["track_xy"].permute(0, 2, 1, 3).contiguous(); vis = batch["track_visible"].bool(); p = sample_field(outputs["P"], xy)
    losses = []
    for b in range(p.shape[0]):
        for t in range(p.shape[2]):
            vv = torch.nonzero(vis[b, t], as_tuple=False).flatten()
            if vv.numel() < 2: continue
            q = p[b, vv, t]; center = q.mean(0, keepdim=True); losses.append(F.smooth_l1_loss(q, center.expand_as(q), beta=0.01))
    return torch.stack(losses).mean() if losses else p.sum() * 0.0


@dataclass(frozen=True)
class LossWeights:
    P: float = 1.0; N: float = 0.25; U_geo: float = 0.05
    Zc_multi_positive: float = 0.10; Zc_dual: float = 0.10; Zc_hard_margin: float = 0.05; Zc_reciprocal_soft: float = 0.02
    P_consistency: float = 0.20; Zf_local: float = 0.10


def total_loss(outputs, batch, epoch: int, warmup_epochs: int = 4, weights: LossWeights = LossWeights()):
    g = geometry_loss(outputs, batch); parts = dict(g)
    total = weights.P * g["P"] + weights.N * g["N"] + weights.U_geo * g["U_geo"]
    if epoch >= warmup_epochs and batch.get("track_visible") is not None:
        c = coarse_correspondence_loss(outputs, batch); f = fine_local_loss(outputs, batch); pc = track_p_consistency_loss(outputs, batch)
        parts.update(c); parts.update(f); parts["P_consistency"] = pc
        total = total + weights.Zc_multi_positive*c["Zc_multi_positive"] + weights.Zc_dual*c["Zc_dual"] + weights.Zc_hard_margin*c["Zc_hard_margin"] + weights.Zc_reciprocal_soft*c["Zc_reciprocal_soft"] + weights.P_consistency*pc + weights.Zf_local*f["Zf_local"]
    parts["total"] = total; return parts
