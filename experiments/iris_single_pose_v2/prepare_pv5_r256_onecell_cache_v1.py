...
def main():
    ...
    if sm.get('record_count')!=1 or sm.get('asset_style_cells')!=1 or sm.get('camera_json_consumed') is not False:
        raise RuntimeError('one-cell stage authority drift')
    ...
    atomic_json(out/'CACHE_MANIFEST.json',
                {'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellCacheManifest.v1',
                 'record_count':1,'asset_style_cells':1,'samples_per_view':S,
                 'camera_json_consumed':False,
                 'records':[{'asset_id':aid,'style':style,'split':'FIT',
                             'asset_dir':str(ar),'truth_path':str(truth),
                             'truth_sha256':sha256_file(truth),
                             'sheet_half_extent':float(r['sheet_half_extent'])}]})
