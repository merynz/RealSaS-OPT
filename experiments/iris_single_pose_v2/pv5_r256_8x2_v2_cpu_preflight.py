...
def make_batch(B=4,H=32,S=64):
    images=torch.rand(B,8,4,H,H); yaw=(torch.arange(8,dtype=torch.float32)*45)[None].repeat(B,1); h=torch.linspace(.53,.62,B)
    xy=torch.rand(B,8,S,2)*1.8-.9; truth=torch.randn(B,8,S,3)*.1; mask=torch.ones(B,8,S,dtype=torch.bool)
...
def main():
    ...
    acc.zero_grad(set_to_none=True); (loss(acc,sl(batch,0,2))/2).backward(); (loss(acc,sl(batch,2,4))/2).backward()
    probes=['p_depth_head.weight','p_s1_stem.0.conv.weight','encoder.s2.0.conv.weight']; max_abs=0.0; max_rel=0.0; rows=[]
    ...
    for n in probes:
        ...
        d=(g1-g2).abs(); ma=float(d.max()); den=float(g1.abs().max())+1e-12; rel=ma/den
        max_abs=max(max_abs,ma); max_rel=max(max_rel,rel); rows.append({'param':n,'max_abs_diff':ma,'relative_to_full_max':rel})
    if max_abs>2e-5 and max_rel>2e-4: raise RuntimeError(f'accumulation equivalence failed: abs={max_abs} rel={max_rel}')
    opt=torch.optim.AdamW([p for p in base.parameters() if p.requires_grad],lr=3e-5,betas=(.9,.95),weight_decay=0.)
    scaler=torch.amp.GradScaler('cuda',enabled=False)
    payload=checkpoint_payload(base,opt,scaler,'CPU_SCHEMA_TEST',0,'TAIL',0)
    required={'model_state','optimizer_state','scaler_state','rng_state'}
    if not required.issubset(payload): raise RuntimeError('V2 resumable checkpoint schema incomplete')
    rep={'schema':'RealSaS.IRISSinglePoseV2.PV5R256EightByTwoCPUPreflight.v2','status':'PASS','checks':{'full_res_output':True,'no_batchnorm':True,'nonzero_dropout_absent':True,'gradient_accumulation_equivalence':'PASS','equivalence_probe_rows':rows,'equivalence_max_abs_diff':max_abs,'equivalence_max_relative':max_rel,'production_microbatch_cells':8,'production_accumulation_steps':2,'resumable_checkpoint_schema':'PASS','scientific_optimizer_steps':0},'trainable_count':len([n for n,p in base.named_parameters() if p.requires_grad])}
