...
def main():
 ...
 seed=work/'MINI_EXTRACTABILITY_SEED_V1.json';fit_stage=work/'stage_fit_v2_256';fit_cache=work/'cache_fit_v2_256';fit_audit=work/'FIT_STAGE_CACHE_AUDIT_V2_256.json';roles=work/'role_manifests';learner=work/'learner';tune_stage=work/'stage_tune_v2_256';tune_cache=work/'cache_tune_v2_256';tune_audit=work/'TUNE_STAGE_CACHE_AUDIT_V2_256.json';compact=work/'MINI_COMPACT_HANDOFF_V1.json'
 ...
 pre={'schema':'RealSaS.IRISSinglePoseV2.MiniPreOptimizerAuthority.v2','status':'FROZEN__OPTIMIZER_AUTHORIZED_FOR_THIS_MINI_ONLY','representation_gate':'P_GEOMETRY_SUFFICIENT','representation_seed_sha256':EXPECTED_REP_SEED_SHA,'panel_asset_id_digest':EXPECTED_PANEL_DIGEST,'membership_canonical_sha256':EXPECTED_MEMBERSHIP_CANONICAL_SHA,'membership_artifact_byte_sha256':sha256_file(a.membership),'mini_seed_sha256':sha256_file(seed),'fit_stage_manifest_sha256':sha256_file(fsm),'fit_cache_manifest_sha256':sha256_file(fcm),'fit_stage_cache_audit_sha256':sha256_file(fit_audit),'role_manifest_sha256':{'FIT_TRAIN':sha256_file(train),'FIT_SELECT':sha256_file(select)},'gpu_capacity_preflight_sha256':sha256_file(a.gpu_preflight_json),'gpu_capacity':capacity,'script_sha256':scripts,'input_resolution':256,'role_counts':ROLE_COUNTS,'normal_policy':'local raster-observation orientation auxiliary target only; forbidden from correspondence ranking and checkpoint selection','tune_policy':'TUNE_FINAL membership frozen, but no TUNE image/truth stage/cache exists before FIT checkpoint freeze','tune_stage_cache_exists_at_optimizer_open':False,'sealed_splits_opened':False,'optimizer_steps_at_freeze':0};atomic_json(work/'MINI_PREOPT_AUTHORITY.json',pre)
