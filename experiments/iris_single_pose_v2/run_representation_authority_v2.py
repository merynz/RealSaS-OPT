...
def main() -> None:
    ap = argparse.ArgumentParser(description="One-command optimizer=0 R0-R3 Representation Authority gate, representation-only staging")
    ...
    work = Path(args.work_dir) if args.work_dir else root / "runs" / "IRIS_SINGLE_POSE_V2_REPRESENTATION_AUTHORITY_V2"
    ...
    stage = work / "stage_representation_v1"
    stage_manifest = stage / "STAGE_MANIFEST.json"
    cache = work / "representation_cache_v1"
    cache_manifest = cache / "CACHE_MANIFEST.json"
    audit = work / "REPRESENTATION_STAGE_CACHE_AUDIT_V1.json"
    ...
    run_authority = work / "RUN_AUTHORITY_V2.json"
    run_complete = work / "RUN_COMPLETE_V2.json"

    scripts = script_hashes(here)
    authority = {
        "schema": "RealSaS.IRISSinglePoseV2.RepresentationRunAuthority.v2",
        "optimizer_steps": 0,
        ...
        "status": "OPTIMIZER_ZERO_REPRESENTATION_ONLY_RUN_AUTHORITY_FROZEN",
    }
    if run_authority.exists():
        old = json.load(open(run_authority, encoding="utf-8"))
        fields = (
            "truth_authority_resolution", "stage_profile", "rgb_staged", "rgb_consumed",
            "expected_panel_asset_id_list_sha256", "settings", "source_authorities", "script_sha256",
        )
        comparable = {k: old.get(k) for k in fields}
        current = {k: authority.get(k) for k in fields}
        if comparable != current:
            raise RuntimeError("existing V2 run authority differs from current source/settings/scripts; choose a new --work-dir")
    else:
    ...
    if not stage_manifest.is_file():
        raise RuntimeError("representation stage manifest absent")
    stage_obj = json.load(open(stage_manifest, encoding="utf-8"))
    if stage_obj.get("stage_profile") != STAGE_PROFILE or stage_obj.get("images_staged") is not False:
        raise RuntimeError("representation-only stage contract drift")
    ...
    if not cache_manifest.is_file():
        raise RuntimeError("representation cache manifest absent")
    cache_obj = json.load(open(cache_manifest, encoding="utf-8"))
    if cache_obj.get("stage_profile") != STAGE_PROFILE or cache_obj.get("rgb_consumed") is not False:
        raise RuntimeError("representation cache contract drift")
    ...
    if not audit.is_file():
        raise RuntimeError("representation stage/cache audit absent")
    audit_obj = json.load(open(audit, encoding="utf-8"))
    if (
        audit_obj.get("status") != "PASS"
        or audit_obj.get("fatal_asset_count") != 0
        or audit_obj.get("asset_count") != EXPECTED_PANEL
        or audit_obj.get("images_required") is not False
        or audit_obj.get("rgb_consumed") is not False
    ):
        raise RuntimeError("representation stage/cache audit did not close PASS on frozen 256")
    ...
    if seed_ids != panel_ids:
        raise RuntimeError("study panel differs from frozen pre-stage seed")
    panel_id_digest = asset_id_list_sha256(panel_ids)
    if panel_id_digest != EXPECTED_PANEL_ASSET_ID_LIST_SHA256:
        raise RuntimeError("post-study panel digest differs from pre-result lock")
    ...
    atomic_json(run_complete, final)
    print(json.dumps({
        "status": final["status"],
        "optimizer_steps": 0,
        "asset_count": final["asset_count"],
        "arm_count": final["arm_count"],
        "stage_profile": final["stage_profile"],
        "rgb_staged": final["rgb_staged"],
        "rgb_consumed": final["rgb_consumed"],
        "panel_asset_id_list_sha256": panel_id_digest,
        "result": str(result),
