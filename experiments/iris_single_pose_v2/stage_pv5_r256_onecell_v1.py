...
def main():
    ...
    if mem.get('asset_count')!=1 or mem.get('asset_style_cells')!=1 or len(mem.get('records',[]))!=1: raise RuntimeError('exact one-cell membership required')
    rec=mem['records'][0]
    if rec.get('split')!='FIT' or rec.get('style')!='cel_clean': raise RuntimeError('membership must be frozen FIT/cel_clean')
    aid=rec['asset_id']; style=rec['style']; src=root/'master'/'assets'/aid
    if not src.is_dir(): raise FileNotFoundError(src)
    (out/'assets').mkdir(parents=True,exist_ok=True); final=out/'assets'/aid
    ...
    try:
        ...
        for v in range(VIEWS):
            ...
            views.append({'view':v,'yaw_deg':YAWS[v],'style':style,'native_sha256':native_hashes[f'V{v}/{style}'],'derivative_512_sha256':deriv_hashes[f'V{v}/{style}'],'input_transform':'PIL_RGBA_BILINEAR_512_to_256'})
        h=native_h_from_paths([src/'renders'/f'V{v}'/f'{style}.png' for v in range(VIEWS)])
        meta={'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellStage.v1','asset_id':aid,'style':style,'split':'FIT','input_resolution':256,'native_scale_source_resolution':1024,'canonical_yaw_deg':YAWS,'sheet_half_extent':h,'camera_json_consumed':False,'teacher_camera_half_extent_consumed':False,'geometry_sha256':sha256_file(src/'primary_geometry.npz'),'raster_sha256':raster_hashes,'native_rgba_sha256':native_hashes,'derivative_512_sha256':deriv_hashes,'views':views}
        atomic_json(tmp/'STAGE.json',meta)
        if final.exists(): shutil.rmtree(final)
        os.replace(tmp,final)
    finally:
        if tmp.exists(): shutil.rmtree(tmp,ignore_errors=True)
    atomic_json(out/'STAGE_MANIFEST.json',{'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellStageManifest.v1','record_count':1,'asset_style_cells':1,'input_resolution':256,'camera_json_consumed':False,'records':[{'asset_id':aid,'style':style,'split':'FIT','asset_dir':str(final),'sheet_half_extent':h}]})
