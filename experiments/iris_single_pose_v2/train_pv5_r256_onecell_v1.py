...
def main():
    ap=argparse.ArgumentParser(description='P-V5 R256 one-cell overfit; CUDA required; no TUNE/sealed arguments')
    ...
    out=Path(a.out_dir); out.mkdir(parents=True,exist_ok=True); (out/'eval').mkdir(exist_ok=True); (out/'checkpoints').mkdir(exist_ok=True)
    ...
    opt=torch.optim.AdamW([p for p in model.parameters() if p.requires_grad],lr=LR,weight_decay=WD,betas=(.9,.95))
    scaler=torch.cuda.amp.GradScaler(enabled=True)

    authority={'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellTrainingAuthority.v1',
               'seed':SEED,'input_resolution':256,'output_field_hw':256,'planned_optimizer_steps':STEPS,
               'candidate_steps':list(CANDIDATES),
               'optimizer':{'name':'AdamW','lr':LR,'betas':[.9,.95],'weight_decay':WD},
               'objective':'FP32 SmoothL1 on camera-forward depth only, beta=0.01',
               'evaluation_authority':'full reconstructed canonical P Euclidean error',
               'target_P_p95':.005,
               'membership':'1 frozen FIT asset x cel_clean',
               'asset_id':cpu_batch['asset_id'][0],'style':cpu_batch['style'][0],
               'augmentation':False,'tune_consumed':False,'sealed_splits_opened':False,
               'camera_json_consumed':False,'trainable_parameters':trainable}
    atomic_json(out/'TRAIN_AUTHORITY.json',authority)
    atomic_json(out/'eval'/'INIT.json',evaluate(model,a.cache_manifest,device))
    ...
    for step in range(1,STEPS+1):
        ...
        if tuple(outputs['P'].shape)!=(1,8,3,256,256):
            raise RuntimeError(f'R256 training field drift: {tuple(outputs["P"].shape)}')
        ...
        if step==1 or step%16==0 or step in CANDIDATES:
            rec={'step':step,'depth_smooth_l1':float(parts['depth_smooth_l1'].detach().cpu()),
            ...
            if step%64==0 or step in CANDIDATES:
                print(f'[r256-onecell] step={step}/{STEPS} depth={rec["depth_smooth_l1"]:.6g} p95={rec["train_P_p95"]:.6g}',flush=True)

        if step in CANDIDATES:
            ev=evaluate(model,a.cache_manifest,device); ev.update(optimizer_steps=step)
            atomic_json(out/'eval'/f'STEP_{step:04d}.json',ev)
            cp=out/'checkpoints'/f'step_{step:04d}.pt'
            torch.save({'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellCheckpoint.v1',
                        'optimizer_steps':step,'model_state':{k:v.detach().cpu() for k,v in model.state_dict().items()}},cp)
            candidates.append({'optimizer_steps':step,'P_p95':ev['P_p95'],'pass_0p005':ev['pass_0p005'],'checkpoint':str(cp)})
            print(json.dumps(candidates[-1],indent=2),flush=True)

    best=min(candidates,key=lambda x:(x['P_p95'],x['optimizer_steps']))
    shutil.copy2(best['checkpoint'],out/'BEST_CHECKPOINT.pt')
    best_eval=json.load(open(out/'eval'/f'STEP_{best["optimizer_steps"]:04d}.json',encoding='utf-8'))
    status='P_V5_R256_ONE_CELL_OVERFIT_PASS' if best_eval['pass_0p005'] else 'P_V5_R256_ONE_CELL_OPTIMIZATION_INSUFFICIENT'
    decision={'schema':'RealSaS.IRISSinglePoseV2.PV5R256OneCellDecision.v1','status':status,
              'selected_step':best['optimizer_steps'],'optimizer_steps':STEPS,
              'selected_P_p95':best_eval['P_p95'],'threshold_P_p95':.005,
              'pass_0p005':best_eval['pass_0p005'],'output_field_hw':256,
              'asset_id':authority['asset_id'],'style':authority['style'],
              'tune_consumed':False,'sealed_splits_opened':False,'camera_json_consumed':False,
              'next_policy':{
                  'P_V5_R256_ONE_CELL_OVERFIT_PASS':'preregister 1 asset x 2 styles R256 overfit only',
                  'P_V5_R256_ONE_CELL_OPTIMIZATION_INSUFFICIENT':'localize R256 learner/optimizer; do not reopen closed P ontology/V5 geometry/free-R256 representation without contradictory evidence'
              }[status]}
    atomic_json(out/'P_V5_R256_ONE_CELL_DECISION.json',decision)
