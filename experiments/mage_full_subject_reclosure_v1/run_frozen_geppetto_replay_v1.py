...
"""Optimizer-free Geppetto replay for a corrected full-subject Mage surface.

This runner deliberately executes the already-promoted Geppetto checkpoint before any
Geppetto retraining is authorized. It rebuilds GSA from a newly emitted signed
zero-surface using exact observation-grounded support, tensorizes the new S lineage,
loads the frozen checkpoint strictly, runs the four frozen diffusion seeds, and lets
Compiler qualification decide whether the old model transfers to the corrected S.

No optimizer is constructed. No checkpoint is written. No old QualifiedSkeletonIR is
reused. Source/teacher geometry is evaluation authority only.
"""
...
SCHEMA = "RealSaS.MageFullSubject.FrozenGeppettoReplay.v1"
...
def _build_corrected_surface(args, cameras, observation_masks):
    zero_path = Path(args.zero_surface)
    zero_sha = _sha(zero_path)
    ...
    surface = rigging_surface_from_scene_first_zero_mesh_v1(
        ...
        authority_label="IRIS_SCENE_FIRST_SIGNED_V3_FULL_SUBJECT_RECLOSURE",
    ...
    if tensor.node_count < 64:
        raise RuntimeError(f"CORRECTED_GSA_NODE_COUNT_DEGENERATE::{tensor.node_count}")
    if tensor.edge_count <= tensor.node_count:
        raise RuntimeError(f"CORRECTED_GSA_TOPOLOGY_DEGENERATE::{tensor.edge_count}")
    if not tensor.raster_valid.any() or not tensor.support.any():
        raise RuntimeError("CORRECTED_GSA_OBSERVATION_SUPPORT_EMPTY")
    return surface, tensor, zero_sha
...
def run(args) -> dict:
    ...
    surface, tensor, zero_sha = _build_corrected_surface(args, cameras, observation_masks)
    ...
    status = (
        "PASS__FROZEN_GEPPETTO_TRANSFERS_TO_CORRECTED_FULL_SUBJECT_S"
        if passed
        else "FAIL__FROZEN_GEPPETTO_DOES_NOT_TRANSFER__RETRAIN_DECISION_REQUIRED"
    )
    qualified_rows = [
        row.get("qualified")
    ...
    report = {
        ...
        "corrected_zero_surface_sha256": zero_sha,
        ...
        "geppetto_retraining_authorized_by_this_result": not passed,
