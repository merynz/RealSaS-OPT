...
def _mock_surface():
    P=np.asarray([[-.4,-.4,0.],[.4,-.4,0.],[.4,.4,0.],[-.4,.4,0.]],np.float64); support=np.ones((4,8),np.uint8); raster=np.zeros((4,8,2),np.float64)
    for i,(x,y,_z) in enumerate(P):
        for v in range(8): raster[i,v]=(x,y)
    base=rigging_surface_from_d2_arrays(P,support,raster,authority_label='MOCK_LEARNED_IRIS_DEPTH_V1')
    ids=[n.surface_id for n in base.surface_nodes]; pairs=((0,1),(1,2),(0,2),(0,3),(2,3)); rel=tuple(SurfaceRelation(f'R:{i}',ids[a],ids[b],'LOCAL_NEIGHBOR',1.0,{'mock_learned_relation':True,'crosses_unknown':False}) for i,(a,b) in enumerate(pairs))
    lineage=content_sha256({'base':base.geometry_lineage_hash,'relations':[r.to_dict() for r in rel]})
    return RiggingSurfaceIR(base.surface_nodes,rel,lineage,base.builder_id,base.schema_version,{**base.metadata,'mock_learned_input':True})
...
def _mock_skin(surface,skeleton):
    ...
    for node in surface.surface_nodes:
        for ji,jid in enumerate(jids): influences.append(SkinInfluenceProposal(node.surface_id,jid,1.0/len(jids)))
...
def run_complete_e2e_v1(output_root=None):
    ...
    if proof.overall_status!='PASS': raise RuntimeError(f'SYNTHETIC_E2E_PROOF_NOT_PASS:{proof.overall_status}')
