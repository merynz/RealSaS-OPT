...
from .arachne_tail_objective_v1 import top_fraction_row_l1_tail_v1
...
def train_arachne_r6_a1_step_v1(
    ...
    *,
    ...
    """Generic A1 optimization with permanent hard-tail consequence pressure.

    The tail term is not a family remediation hook. It is a family/semantic-ID
    agnostic top-fraction row-L1 objective over the current admitted surface and
    is part of the base A1 contract. This prevents low-area but mechanically
    important skin rows from being hidden by a good mean reconstruction loss.
    """
    ...
    if min(latent_nll_weight, reconstruction_weight, deformation_weight, tail_weight) < 0 or latent_nll_weight == 0:
        raise ValueError("invalid Arachne A1 loss weights")
    if not (0.0 < float(tail_fraction) <= 1.0):
        raise ValueError("invalid Arachne A1 tail fraction")
    ...
    nll = 0.5 * torch.exp(-2.0 * ls) * residual.square() + ls
    latent_nll = nll[valid_latent].mean()
    ...
    total = (
        ...
        + deformation_weight * deformation["deformation_mse"]
        + tail_weight * tail
    )
    if not torch.isfinite(total):
        raise ValueError("non-finite Arachne A1 loss")
    total.backward()
    optimizer.step()
    return {
        "total": float(total.detach().cpu()),
        "latent_nll": float(latent_nll.detach().cpu()),
        "reconstruction": float(reconstruction["total"].detach().cpu()),
        "tail_row_l1": float(tail.detach().cpu()),
        "deformation_rms": float(deformation["deformation_rms"].detach().cpu()),
        "uncertainty_mean": float(torch.exp(ls[valid_latent]).mean().detach().cpu()),
    }
