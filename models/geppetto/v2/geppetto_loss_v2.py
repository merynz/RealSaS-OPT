...
def canonical_geometry_assignment_v2(primary_positions, teacher_positions) -> tuple[np.ndarray, np.ndarray]:
    """Geometry-only anonymous assignment with exact integer secondary authority."""
    ...
    def __call__(self, output: GeppettoRawOutputV2, targets: Sequence[GeppettoTeacherTargetV1], surface_positions_normalized: torch.Tensor, valid_mask: torch.Tensor):
        ...
        for b, target in enumerate(targets):
            ...
            for mapping in variants:
                ...
                topology_candidates.append((self.weights.root * root_loss + self.weights.parent * parent_loss, root_loss, parent_loss))
            best = min(range(len(topology_candidates)), key=lambda i:(float(topology_candidates[i][0].detach().cpu()),float(topology_candidates[i][1].detach().cpu()),float(topology_candidates[i][2].detach().cpu())))
            total["root"] += topology_candidates[best][1]; total["parent"] += topology_candidates[best][2]
