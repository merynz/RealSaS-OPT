...
class CameraAwareViewTokenBankV3(nn.Module):
    ...
    def forward(
        ...
        z = self.norm(z)
        # No learned view-id or view-position embedding. Joint view+camera permutation
        # only permutes the memory sequence; scene cross-attention is invariant to it.
...
class ContinuousSignedSurfaceFieldV3(nn.Module):
    ...
    The scalar head is intentionally unconstrained. Surface extraction owns the zero
    level set. This is the promoted correction over the rejected UDF+ray-argmin seam.
    """
