...
from .config_v4 import ArachneSkinFieldConfigV4

class FrequencyXYZEmbeddingV4(nn.Module):
...
class SelfAttentionBlockV4(nn.Module):
    def __init__(self, dim: int, heads: int, ratio: int, dropout: float):
        ...
        self.ff = FeedForwardV4(dim, ratio, dropout)
...
class CrossAttentionBlockV4(nn.Module):
...
class LatentSetEncoderV4(nn.Module):
    def __init__(self, query_dim: int, memory_dim: int, width: int, heads: int, layers: int, ratio: int, dropout: float):
        super().__init__()
        self.cross = CrossAttentionBlockV4(query_dim, memory_dim, width, heads, ratio, dropout)
        self.blocks = nn.ModuleList([SelfAttentionBlockV4(width, heads, ratio, dropout) for _ in range(layers)])
...
class ScalarFieldQueryDecoderV4(nn.Module):
    def __init__(self, query_dim: int, latent_dim: int, width: int, heads: int, layers: int, ratio: int, dropout: float):
        ...
        self.memory_blocks = nn.ModuleList([SelfAttentionBlockV4(width, heads, ratio, dropout) for _ in range(layers)])
        self.query_cross = CrossAttentionBlockV4(query_dim, width, width, heads, ratio, dropout)
        self.out_norm = nn.LayerNorm(width, eps=1e-5)
...
class FiniteScalarQuantizerV4(nn.Module):
...
class SkinFieldCodecV4(nn.Module):
    """Arachne sparse skin-field codec. Joint/pair geometry never enters this decoder."""
    def __init__(self, config: ArachneSkinFieldConfigV4 = ArachneSkinFieldConfigV4()):
        super().__init__()
        config.validate()
        self.config = config
        self.xyz_embed = FrequencyXYZEmbeddingV4(config.num_frequencies, config.include_input, config.include_pi)
        self.field_queries = nn.Parameter(torch.randn(config.field_tokens, config.encoder_width) * 0.02)
        self.field_encoder = LatentSetEncoderV4(
        ...
        self.condition_encoder = LatentSetEncoderV4(
        ...
        self.fsq = FiniteScalarQuantizerV4(config.latent_channels, config.fsq_levels)
        self.decoder = ScalarFieldQueryDecoderV4(
...
def scalar_field_loss_v4(
    ...
    *,
    ...
    """Sparse-aware reconstruction loss following the SkinTokens paper's BCE+MSE+Dice design."""
