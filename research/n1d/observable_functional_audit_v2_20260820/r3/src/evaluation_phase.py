...
def evaluate_family(root: Path, family: int, episode: str, obs_dir: Path, sidecar: Path):
    ...
    for i in range(64):
        ...
        # Frozen V8/V5 route modules use OBS_ALPHA=.75; this rank is descriptive only.
    ...
    for i in witness_ids:
        ...
        rows.append({
            ...
            "descriptive_geometry": {
                "F_delta_NRMS": float(np.linalg.norm(np.asarray(base_g["F_delta_normalized"])[i] - np.asarray(swap_g["F_delta_normalized"])[i]) / (np.linalg.norm(np.asarray(swap_g["F_delta_normalized"])[i]) + 1e-8)),
            },
        })
    return {
