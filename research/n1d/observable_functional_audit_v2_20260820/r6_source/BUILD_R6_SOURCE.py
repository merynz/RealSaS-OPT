...
LOAD_ANCHOR = "    assert spec.loader is not None\n    spec.loader.exec_module(mod)\n"
LOAD_REPLACEMENT = "    assert spec.loader is not None\n    sys.modules[name] = mod\n    spec.loader.exec_module(mod)\n"
...
def build(base_root: Path, out_root: Path):
    ...
    if text.count(IMPORT_ANCHOR) != 1 or text.count(LOAD_ANCHOR) != 1:
        raise RuntimeError("R6 patch anchor mismatch")
    text = text.replace(IMPORT_ANCHOR, IMPORT_REPLACEMENT, 1)
    text = text.replace(LOAD_ANCHOR, LOAD_REPLACEMENT, 1)
    dst.write_text(text)
    (out_root / "tests" / "test_imports.py").write_text(IMPORT_TEST)
    print("R6_SOURCE_BUILD_PASS")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("usage: BUILD_R6_SOURCE.py <checked-out-base-root> <out-root>")
