...
SCHEMA = "RealSaS.N1D.Rank2GlobalRelationalWorld.V0.TruthEvaluation.v1"
V0_SOURCE_SHA256 = "7915cb4a4e1bb3a83645526c61e55d4180d20c37e4541b3771b6e6eb4c11a11c"
...
def evaluate_family(root: Path, science_dir: Path, state_path: Path, selection_path: Path, sidecar: Path, family: int, episode: str):
    ...
    if sel.get("truth_access") != "NONE": raise RuntimeError("V0 observable selection truth contract invalid")
    if sel.get("source_sha256") != V0_SOURCE_SHA256: raise RuntimeError("V0 source authority mismatch")
    famrec=next((x for x in sel["families"] if int(x["family"])==int(family)), None)
    if famrec is None: raise RuntimeError(f"family {family} absent from V0 selection")
    ...
    if gidx.shape != (64,): raise RuntimeError("V0 selection must have 64 candidate indices")
    for i in range(64):
    ...
    # Full V0 world gets normals/visibility only from the real observable raster/model decorator.
    ...
    # Truth opens only here, after V0 source and observable selection are frozen.
    ...
    # Secondary all reliable+active summary; no witness redefinition.
