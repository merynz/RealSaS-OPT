...
SCHEMA='RealSaS.N1D.Rank2R.V3C.TruthEvaluation.v1'
WORLD_AUTH_SHA256='1161ec646741f03184cacf0c8f57336065f2d3aa5be21852528162416fcfe40f'
...
def evaluate_family(root,science_dir,state_path,world_path,world_authority_path,sidecar,family,episode):
    ...
    if sha256_file(world_authority_path)!=WORLD_AUTH_SHA256: raise RuntimeError('V3C world authority SHA mismatch')
    auth=json.loads(world_authority_path.read_text())
    if auth.get('truth_access')!='NONE' or auth.get('status')!='FROZEN_PRE_TRUTH': raise RuntimeError('V3C world authority not pre-truth frozen')
    if auth.get('source_sha256')!=V3C_SOURCE_SHA256 or auth.get('observable_panel_manifest_sha256')!=OBS_PANEL_SHA256: raise RuntimeError('V3C source/panel authority mismatch')
    ...
    PA=np.asarray(z['P_A'],np.float32); PB0=np.asarray(z['P_B'],np.float32); PBv=np.asarray(wz['P_B_V3C'],np.float32)
    ...
    ev=load_module('v3c_r6_eval_exact',science_dir/'evaluation_phase.py')
    mm=load_module('v3c_r6_metrics_exact',science_dir/'mechanics_metrics.py')
    obs=load_module('v3c_r6_observable_exact',science_dir/'observable_phase.py')
    ...
    # Truth opens only here, after all V3-C world bytes are verified frozen.
