...
"""Knight DEMO_WITNESS Geppetto mechanically-meaningful fresh-fit runner.

Consumes only the qualified Stage15 RiggingSurfaceIR as learner input.
Teacher mechanics remain training/evaluation-only. Free-running evaluation
receives no teacher target. The target is no longer an aesthetic/minimal core:
it preserves skin-supported articulation, structural bridges, one root-motion
anchor, terminal mechanical extensions and explicit terminal tips.

PASS requires three consecutive full checks. Every check still requires all four
frozen diffusion seeds to satisfy the unchanged geometric/topology thresholds.
"""
...
SCHEMA = "RealSaS.KnightGeppettoDemoFitMechanicalRig.v3"
SEED = 20260907
MAX_STEPS = 16384
CHECK_EVERY = 64
TERMINAL_CHECKS = 3
...
def _load_target(npz_path: Path, report_path: Path) -> tuple[MechanicallyMeaningfulRigTargetV2, dict]:
    report = _load_json(report_path)
    if report.get("schema") != "RealSaS.KnightGeppettoMechanicallyMeaningfulRigTarget.v2":
    ...
    with np.load(npz_path, allow_pickle=False) as data:
        ...
        target = MechanicallyMeaningfulRigTargetV2(
            ...
            role_codes=np.asarray(data["role_codes"], dtype=np.int64),
        )
    got = target_content_sha256_v2(target)
    ...
    if str(report.get("selection_rule") or "") != TARGET_RULE:
        raise RuntimeError("GEPPETTO_KNIGHT_TARGET_RULE_DRIFT")
    if dict(report.get("role_counts") or {}) != target.role_counts:
        raise RuntimeError("GEPPETTO_KNIGHT_TARGET_ROLE_COUNTS_DRIFT")
...
def _check(
    ...
    *,
    ...
    rows = []
    print(f"GEPPETTO_KNIGHT_CHECK_BEGIN step={int(step)} seeds={list(DIFFUSION_SEEDS)}", flush=True)
    with torch.no_grad():
                ...
                + json.dumps(
                    {
                        ...
                        "matched_mae_norm": row.get("matched_mae_norm"),
                        "matched_p95_norm": row.get("matched_p95_norm"),
                        "root_accuracy": row.get("root_accuracy"),
                        "parent_accuracy": row.get("parent_accuracy"),
                    },
                    sort_keys=True,
                ),
                flush=True,
...
def run(args) -> dict:
    ...
    target_policy_path = Path(args.target_policy).expanduser().resolve()
    ...
    target_policy = _load_json(target_policy_path)
    if target_policy.get("schema") != "RealSaS.GeppettoMechanicallyMeaningfulRigPolicy.v1":
        raise RuntimeError("GEPPETTO_TARGET_POLICY_SCHEMA_DRIFT")
    if target_policy.get("status") != "FROZEN_FOR_KNIGHT_DEMO_REFIT_BEFORE_OPTIMIZER":
        raise RuntimeError("GEPPETTO_TARGET_POLICY_NOT_FROZEN")
    if str(target_policy.get("target_rule") or "") != TARGET_RULE:
        raise RuntimeError("GEPPETTO_TARGET_POLICY_RULE_DRIFT")
    pass_policy = dict(target_policy.get("pass_policy") or {})
    if int(pass_policy.get("required_consecutive_full_passes", -1)) != TERMINAL_CHECKS:
        raise RuntimeError("GEPPETTO_TARGET_POLICY_TERMINAL_CHECK_DRIFT")
    experiment_prereg = _load_json(experiment_prereg_path)
    if experiment_prereg.get("schema") != "RealSaS.KnightGeppettoDemoFitPreregistration.v2":
    ...
    expected_bindings = {
        ...
        "teacher_target_content_sha256": target_content_sha256_v2(target),
        "target_ontology_policy_sha256": _sha(target_policy_path),
    }
    ...
    preflight = {
        ...
        "tensorization_hash": tensor.tensorization_hash,
        "certificate_hash": tensor.certificate_hash,
        "target_content_sha256": target_content_sha256_v2(target),
        "target_count": target.count,
        "target_role_counts": target.role_counts,
        "target_rule": TARGET_RULE,
        "target_policy_sha256": _sha(target_policy_path),
    ...
    receipt = {
        ...
        "metadata": {
            ...
            "required_consecutive_full_passes": TERMINAL_CHECKS,
            "target_rule": TARGET_RULE,
            "target_role_counts": target.role_counts,
            "selected_proposal_seed_predeclared": SELECTED_PROPOSAL_SEED,
            "runtime_environment": _runtime_environment(),
